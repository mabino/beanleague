...
async def create_team(team_in: TeamCreate, db: aiosqlite.Connection = Depends(get_db)):
    ...
    if not league:
        raise HTTPException(
            ...
            detail=f"League with Season Code '{season_code}' not found."
        )

    # 2. Check team count
    c_cur = await db.execute("SELECT COUNT(*) FROM teams WHERE league_id = ?", (league["id"],))
    count = (await c_cur.fetchone())[0]
    ...
    # 3. Generate unique manager code PIN
    team_id = f"team-{uuid.uuid4().hex[:8]}"
    manager_code = None
    for _ in range(10):
        code_candidate = generate_manager_code()
        chk = await db.execute("SELECT 1 FROM teams WHERE manager_code = ?", (code_candidate,))
        if not await chk.fetchone():
            manager_code = code_candidate
            break
...
async def get_my_roster(team: Dict[str, Any] = Depends(get_current_team), db: aiosqlite.Connection = Depends(get_db)):
    """
    Fetches the authenticated manager's team and roster with player stats.
    """
    team_id = team["id"]
    
    # Fetch roster with full player and scoring details
    cursor = await db.execute(
        """
...
async def save_my_roster(
    ...
    # 3. Transactionally replace team roster
    ...
    for item in player_items:
        await db.execute(
            """
            INSERT INTO rosters (team_id, player_id, is_starting_xi, is_captain, slot_position, slot_index)
            VALUES (?, ?, ?, ?, ?, ?)
            """,
            (
                team_id,
    ...
    # 4. Recalculate team fantasy score
...
async def get_team_public(team_id: str, db: aiosqlite.Connection = Depends(get_db)):
    """
    Public view of any team's pitch view and players for leaderboard scouting.
    """
    t_cur = await db.execute(
        """
        SELECT t.id, t.team_name, t.formation, t.total_points, l.name as league_name, l.season_code, l.salary_cap
        FROM teams t
        JOIN leagues l ON t.league_id = l.id
        WHERE t.id = ?
        """,
    ...
    players = [dict(r) for r in await r_cur.fetchall()]
